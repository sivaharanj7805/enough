# Tended — Product Requirements Document
...
Tended does this in 25 minutes for $7 in API costs:
...
**Budget:** Charges clients $2K-5K per content audit. Tended at $349/month for 3 sites is a fraction of one client engagement.
...
**Conversion trigger:** Tended produces in 25 minutes what takes 2 weeks. The PDF report is ready to rebrand and deliver.
...
| Competitor | Price | Overlap | Tended's advantage |
|---|---|---|---|
| **MarketMuse** | $270-$545+/mo | Content intelligence, topic modeling, content briefs | MarketMuse tells you "write about X." Tended tells you "merge these two posts, here's the redirect URL, here's the new meta description." Plus the ecosystem visualization. |
| **Entail AI** | $200+/mo | Topical authority graphs | Entail shows abstract network diagrams. Tended shows a living ecosystem with terrain, trees, and health scores. |
| **Surfer SEO Content Audit** | $129-$219/mo (bundled) | Monitors positions, suggests refreshes | Growing threat — massive distribution. But Surfer audits individual pages. Tended audits the relationship between pages (cannibalization, clustering, consolidation). |
...
| **Manual agency audit** | $2K-5K one-time | Full content audit | Static PDF, outdated immediately. Tended is a living dashboard that updates weekly. |

### 3.3 Positioning statement

For content marketers at B2B SaaS companies who need to find and fix cannibalization, decay, and dead weight in their blog, Tended is a content ecosystem intelligence platform that analyzes your entire blog in 25 minutes, finds every post competing against another, and tells you exactly what to merge, redirect, and rewrite — unlike Ahrefs and Semrush which identify problems but leave you to figure out the fix, or agencies that charge $5K for a static audit that's outdated tomorrow.
...
**Scale tier:** White-label option (client's branding instead of Tended branding).
...
```
1. Visitor lands on tended.app (source: organic, Indie Hackers, r/SEO, cold DM)
...
9. CTA in PDF: "See the full analysis — subscribe to Tended"
...
```
1. User opens tended.app → lands on /today (daily command center)
...
8. Returns to Tended → marks as done
...
**Key metric:** DAU/MAU ratio. Target: >30% (user opens Tended at least 2x/week).
...
| 17 | Configure Resend (SPF/DKIM/DMARC for tended.app domain) | 2h |
...
`/wrapped` exists. It generates a weekly shareable card (LinkedIn-optimized). This is a growth hack (users share → their network sees Tended branding).
