"""End-to-end pipeline test: SEO marketing blog (backlinko.com).

Brian Dean's Backlinko — a well-known SEO/marketing blog with heavy keyword
overlap across link-building, on-page SEO, and content marketing topics.
Ideal for testing cannibalization detection and topic clustering.
...
logger = logging.getLogger('e2e_seo_blog')

# Target: Brian Dean's SEO blog — high topic overlap in link-building, SEO, content marketing
DOMAIN = os.environ.get('E2E_DOMAIN', 'backlinko.com')
...
async def run_e2e():
    ...
    logger.info("=" * 70)
    logger.info(f"E2E TEST: {DOMAIN} (SEO/marketing blog)")
    logger.info("=" * 70)
    ...
    logger.info("=" * 70)
    logger.info("STEP 11: GENERATE AUDIT REPORT + PDF")
    logger.info("=" * 70)
    try:
        t = time.time()
        ...
        logger.info(f"\n  Headline: {audit_data['headline']}")
        logger.info(f"\n  Key Findings:")
        for f in audit_data.get('key_findings', []):
            logger.info(f"    - {f}")

        logger.info(f"\n  Top Clusters:")
        for c in audit_data.get('top_clusters', []):
            logger.info(f"    [{c['ecosystem_state']}] {c['label']} — {c['post_count']} posts, health {c['health_score']}/100")

        logger.info(f"\n  Worst Posts:")
        for p in audit_data.get('worst_posts', []):
            logger.info(f"    {p['health_score']}/100 [{p['role']}] {p['title'][:55]}")
            if p.get('issue'):
                logger.info(f"      Issues: {p['issue']}")

        logger.info(f"\n  Best Posts (Pillars):")
        for p in audit_data.get('best_posts', []):
            logger.info(f"    {p['health_score']}/100 [{p['role']}] {p['title'][:55]}")

        logger.info(f"\n  Top Cannibalization Pairs:")
        for cp in audit_data.get('top_cann_pairs', []):
            logger.info(f"    [{cp['severity']}] {cp['overlap_score']:.3f} — {cp['post_a_title'][:30]} ↔ {cp['post_b_title'][:30]}")

        logger.info(f"\n  Top Recommendations:")
        for r in audit_data.get('top_recs', []):
            logger.info(f"    [{r['rec_type']}] {r['title']}")
            if r.get('summary'):
        ...
        logger.info(f"\nPDF generated: {len(pdf_bytes) // 1024}KB → {pdf_path}")

        # Also save audit data as JSON for reference
        json_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
            f"{DOMAIN.replace('.', '-')}-audit-data.json"
        )
        with open(json_path, 'w') as f:
            json.dump(audit_data, f, indent=2, default=str)
        logger.info(f"Audit data JSON saved → {json_path}")
    ...
    if all_ok:
        logger.info("\n*** ALL STEPS PASSED — E2E pipeline is fully functional ***")
    else:
